...
import type { ComputedOptions, ConcreteComponent, MethodOptions } from 'nuxt/dist/app/compat/capi';
...
interface Props extends StrapiFetchOptions<DataT> {
  id: string | ConcreteComponent<{}, any, any, ComputedOptions, MethodOptions, {}, any>; // key to ensure multiple fetches in the same route path
...
<template>
  <component :is="on" ref="container">
    <slot v-if="response.status === 'pending'" name="loading">
    ...
    <template v-else-if="response.data">
      ...
      <Pagination
        v-if="
          query.pagination?.pageParam && response.meta?.pagination.pageCount && response.meta?.pagination.pageCount > 1
        "
